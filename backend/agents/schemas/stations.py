...
STATIONS_EXTRACT_SYSTEM = (
	...
	"elles). Ne remonte JAMAIS de toi-même une plainte précise vers une grande spécialité "
	"(ex. 'j'ai mal au dos' → 'mal de dos', PAS 'rhumatologie' ; 'j'ai de l'acné' → 'acné', "
	"PAS 'dermatologie') : préfère toujours le terme précis le plus proche de ce que dit le "
	"client. En revanche, si le client cite LUI-MÊME une spécialité ou un domaine "
	"(ex. 'rhumatologie', 'dermatologie', 'phlébologie'), reprends ce terme tel quel : ne "
	"le remplace pas et ne le rejette pas. N'invente et n'ajoute jamais de précision que le client n'a pas donnée. "
